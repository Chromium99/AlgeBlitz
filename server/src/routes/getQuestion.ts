...
export default async function getQuestion(server: FastifyInstance) {
  server.get<{ Params: { category: string; difficulty: string; uid: string } }>(
    "/question/:category/:difficulty/:uid",
    async (request, reply) => {
      try {
        //accessing query params
        const { category, difficulty, uid } = request.params;
        //getting number of questions using 'count' property of prisma
        const numberOfQuestions = await prisma.questionInfoTable.count({
          //checks number of questions of given params
          where: {
        ...
        async function questionGetter(maxTries = 5) {
          try {
            ...
            let randomIndex =
              numberOfQuestions > 1
                ? Math.floor(Math.random() * numberOfQuestions)
                : 0;
            const randomQuestion = await prisma.questionInfoTable.findMany({
              //select a random question of given params using random value
              where: {
                category: category,
            ...
            const alreadySolvedChecker =
              await prisma.problemSolvedTable.findMany({
                where: {
